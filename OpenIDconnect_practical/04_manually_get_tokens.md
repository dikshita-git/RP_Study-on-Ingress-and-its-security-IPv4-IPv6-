...
```
    ...
    ▶️These annotations in the ingress fixed the error:
```
	annotations:
...
response_type=code + scope=openid + grant_type=authorization_code
...
#### ii) Authentication Request Validation
- performed by keycloak internally


#### iii) Authorization Server Authenticates End-User
- the user logs in

#### iv) Successful Authentication Response
